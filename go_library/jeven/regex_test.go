...
func TestWithValue(t *testing.T) {
	// r := regexp.MustCompile("ID\\-(\\w*)\\-(\\w*)")
	// s := "ID-18374-NDUSHP ID-3823-NJUS ID-893-JJUUS"
	//
	// strings := r.FindAllStringSubmatch(s, -1)
	// fmt.Println(strings)

	//  "/iam/hello/test" => "iam"
	var value = "${iam.accessToken}"
	compile := regexp.MustCompile("\\$\\{(.*)\\}")
	submatch := compile.FindStringSubmatch(value)
	println(submatch)

	var value2 = "iam.accessToken"
	compile2 := regexp.MustCompile("\\$\\{(.*)\\}")
	submatch2 := compile2.FindStringSubmatch(value2)
	println(submatch2)
}
...
var internalPathReg = regexp.MustCompile(",\\s*(https?://.*?)/.*")
var externalPathReg = regexp.MustCompile("(https?://.*?)/.*")

func TestUrl(t *testing.T) {
	value := "https://192.168.159.129:8080/realms/master,http://test:8080/realms/master"
